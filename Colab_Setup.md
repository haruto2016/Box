...
!echo "✅ Installation Complete! (Ultimate Persistence Mode)"
```

## 手順 3: サーバー起動＆トンネル開通 (Ultimate & Persistent)
次のセルに以下のコードを貼り付けて実行します。
**実行時にGoogleドライブのマウント許可を求められるので、承認してください。**

```python
# セル 2: デスクトップストリーミングの開始 (Persistence Mode)
import time
import os
import re
from google.colab import drive

# 1. Googleドライブをマウントしてデータを永続化
print("Mounting Google Drive for Persistence...")
drive.mount('/content/drive')
PERSIST_DIR = "/content/drive/MyDrive/WebBox_Data"
os.makedirs(f"{PERSIST_DIR}/Desktop", exist_ok=True)
os.makedirs(f"{PERSIST_DIR}/chrome-profile", exist_ok=True)
os.makedirs(f"{PERSIST_DIR}/Home", exist_ok=True)

# 2. フォルダをDriveにシンボリックリンク（実体はDriveに保存される）
def link_dir(src, dst):
    if os.path.islink(src): os.unlink(src)
    elif os.path.exists(src): os.system(f"rm -rf {src}")
    os.symlink(dst, src)

link_dir("/root/Desktop", f"{PERSIST_DIR}/Desktop")
link_dir("/root/.chrome-profile", f"{PERSIST_DIR}/chrome-profile")

# Chromeショートカットの再作成（プロファイルをDriveに向ける）
...
print("Starting KasmVNC Server (No Login Mode)...")
...
if url:
    print(f"\n✅ 接続準備完了！ (Persistence & No-Login Mode)")
    print(f"👉 URL: {url}")
    print(f"\n👆 上記URLをコピーして、WebBoxに入力してください。")
    print(f"※ ログイン画面はスキップされます。")
else:
    print("\n❌ 取得失敗。")
