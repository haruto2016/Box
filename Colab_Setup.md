...
# Google Chromeのショートカットをデスクトップに作成（修正版：-eフラグを追加）
...
# 音声ストリーミングサーバーの起動 (Port 8080)
print("Starting Audio Streamer...")
os.system("nohup ffmpeg -f pulse -i default -f mp3 -acodec libmp3lame -ab 128k -listen 1 http://0.0.0.0:8080 > audio.log 2>&1 &")
...
print("Starting Cloudflare Tunnels (Video & Audio)...")
os.system("nohup /usr/local/bin/cloudflared tunnel --url http://localhost:6080 > cloudflared.log 2>&1 &")
os.system("nohup /usr/local/bin/cloudflared tunnel --url http://localhost:8080 > cloudflared_audio.log 2>&1 &")
